...
class WeatherPictogram(
    ...
    fun getAnimatedPictogram(): Int {
        return animatedIconMapping[code] ?: R.raw.not_available
    }

    private fun getAnimatedPictogram(currH: Int, riseH: Int, setH: Int): Int {
        return animatedIconMapping[code.mod(1000) + (if (currH in (riseH + 1)..setH) 1000 else 2000)] ?: R.raw.not_available
    }

    fun getAnimatedPictogram(t: LocalDateTime, sunTimes: SunRiseSunSet): Int {
        return getAnimatedPictogram(t.hour, sunTimes.riseH, sunTimes.setH)
    }
}
